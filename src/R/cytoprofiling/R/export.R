...
#' @param split_assays If TRUE, split the transcript and protein barcoding targets into separate Seurat assays ("RNA" and "Protein", respectively).
...
cytoprofiling_to_seurat <- function(df, panel_json, use_gene_names = FALSE, split_assays = FALSE) {
  ...
  if (use_gene_names) {
    rownames(transcript_counts) <- sapply(rownames(transcript_counts), function(x) {
  ...
  if (split_assays) {
    ...
    if (use_gene_names) {
      rownames(protein_counts) <- sapply(rownames(protein_counts), function(x) {
        strsplit(x, ".", fixed = TRUE)[[1]][1]
      }, USE.NAMES = FALSE)
    }
